...
export -f install_brew_formula

# Install or upgrade a brew cask
# Parameters: $1 (required ) - Cask
...
# Install or update asdf plugins
# Parameters: $1 (required ) - Plugin, $2 (required) - Versions, $3 (optional) - Repo
function install_asdf {
  ...
  if ! asdf plugin-add "$1" "$repo" > /dev/null; then

    for el in ${array[@]}
    do
      asdf install $1 $el || true
    done
    ...
    if [[ ${2} == "latest" ]]; then
      ...
      version=${v//[[:blank:]]/}
      echo "$version"
      asdf global ${1} ${version} || true
      asdf reshim ${1} ${version} || true
    else
      echo "$2"
      asdf global $1 $2 || true
      asdf reshim $1 $2 || true
    fi
  fi
}
